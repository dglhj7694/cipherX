...
.cm-card{border-radius:12px;padding:14px;margin:6px 0;border:1px solid rgba(255,255,255,.06);background:rgba(255,255,255,.02)}
.cm-vote-buy{color:#34D399;font-weight:800}
.cm-vote-sell{color:#F87171;font-weight:800}
.cm-vote-neutral{color:#FCD34D;font-weight:800}
.cm-vote-abstain{color:#475569;font-weight:600}
.veto-badge{display:inline-block;padding:3px 8px;border-radius:6px;font-size:.7rem;font-weight:700;background:rgba(239,68,68,.15);color:#FCA5A5;margin:2px}
.context-badge{display:inline-block;padding:4px 10px;border-radius:8px;font-size:.78rem;font-weight:700;margin:2px}
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  JT: Layer Caps + Committee Ensemble Parameters
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
class JT:
    # ── Layer caps (내부 계산용, 기존과 동일) ──
    ...
    # ── Committee Ensemble Parameters (신규) ──
    # 위원회 정규화 계수 (각 위원회 net score ÷ NORM → -100~+100)
    TREND_NORM=18.;MOMENTUM_NORM=10.;MONEY_NORM=12.;STRUCTURE_NORM=10.;LEADING_NORM=25.
    # 앙상블 판단 임계치 (정규화된 합산 점수 기준)
...
# ── Context-Adaptive Weights (핵심: 상황별 위원 비중) ──
#                         Trend  Momentum  Money  Structure  Leading
CONTEXT_WEIGHTS = {
...
def compute_vp(h,l,c,v,lb=20,nb=30,step=3):
...
def compute_ut_bot(c,h,l,atr,key_value=1):
    ...
    for i in range(1,n):
        if np.isnan(xatr[i]) or np.isnan(cv[i]):
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  detect_all_signals (기존 동일 + 마지막에 committee 호출)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def detect_all_signals(df):
    ...
    # ═══ Phase 4: Combined Scan + 10-Layer Scores + 5-Committee Ensemble ═══
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  10-Layer Scores (시각화용, 판단 로직 분리)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def compute_10layer_scores(df,vol_ratio,hma_r_v):
    """10-Layer 점수만 계산 (차트 시각화용). 판단은 Committee에서."""
    ...
    blag+=np.where(regime.values<=-1,-1.5,0)
    ...
    slag_+=np.where(regime.values>=1,-1.5,0)
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  🏛️ 5-COMMITTEE ENSEMBLE JUDGMENT SYSTEM (핵심 신규)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def _committee_money(df, N):
    """💰 자금흐름 위원회 — CMF, OBV, MFI, Volume, RSI_MFI"""
    ...
    # OBV 추세 (20일 이동평균 대비)
    obv=N('OBV');obv_ma=obv.rolling(20,min_periods=10).mean()
    obv_ratio=(obv-obv_ma)/(obv_ma.abs()+1e-10)*100
    score+=np.clip(obv_ratio*.3, -20, 20)

    # OBV 방향 일관성 (5일)
    obv_dir_streak=_vs(obv>obv.shift(1))
    obv_dn_streak=_vs(obv<obv.shift(1))
    score+=np.where(obv_dir_streak>=5, 10, np.where(obv_dn_streak>=5, -10, 0))
    ...
    # RSI_MFI 복합 (-50~+50)
    rmfi=N('RSI_MFI')
    score+=np.clip(rmfi*.8, -15, 15)

    # 거래량 (확인/부정)
    vol=df['Volume'];vol_avg=vol.rolling(50,min_periods=10).mean()
    vr=vol/(vol_avg+1e-10)
    price_dir=np.where(C>C.shift(1), 1, np.where(C<C.shift(1), -1, 0))
    # 상승+고거래량=강세, 하락+고거래량=약세
    vol_confirm=np.where(vr>=2, price_dir*15, np.where(vr>=1.5, price_dir*8, 0))
    score+=vol_confirm

    # 정규화
    norm_score=(score/JT.MONEY_NORM*100).clip(-100,100)

    # 확신도: 거래량 높을수록 확신 (자금흐름 신뢰도)
    vol_conviction=np.clip(vr.values*30, 10, 60)
    # CMF가 극단이면 확신 부스트
    cmf_conviction=np.clip(cmf.values.copy()*100, 0, 30)
    conviction=np.clip(vol_conviction + np.abs(cmf_conviction), 10, 95)

    return norm_score, pd.Series(conviction, index=idx)


def _committee_structure(df, N):
    """🏗️ 구조 위원회 — BB, VP, 캔들패턴, 지지/저항"""
    idx=df.index;C=df['Close'];O=df['Open']
    score=pd.Series(0., index=idx)
    ...
    # BB 반등/이탈
    bb_score+=np.where((pb<.1)&(C>O), 10, 0)  # BB 하단+양봉=반등
    bb_score+=np.where((pb>.9)&(C<O), -10, 0)  # BB 상단+음봉=하락
    score+=bb_score
    ...
    score+=np.where(((C-val_).abs()/(C+1e-10)<.02)&(C>O), 10, 0)  # VAL 지지
    score+=np.where(((vah-C).abs()/(C+1e-10)<.02)&(C<O), -10, 0)  # VAH 저항

    # 캔들패턴 (강력한 것만)
    F=lambda c:df.get(c,pd.Series(False,index=idx)).fillna(False)
    candle_buy=F('Morning_Star').astype(float)*20+F('Bullish_Engulfing').astype(float)*15+F('Hammer').astype(float)*12+F('Outside_Bullish').astype(float)*10+F('Doji_Bullish').astype(float)*5
    candle_sell=F('Evening_Star').astype(float)*20+F('Bearish_Engulfing').astype(float)*15+F('Shooting_Star').astype(float)*12+F('Outside_Bearish').astype(float)*10+F('Doji_Bearish').astype(float)*5
    score+=candle_buy-candle_sell
    ...
    # 확신도: 구조적 증거 수
    n_evidence=(
        (pb<.2).astype(float)+(pb>.8).astype(float)+
        (candle_buy>0).astype(float)+(candle_sell>0).astype(float)+
        (((C-val_).abs()/(C+1e-10)<.03)|(((vah-C).abs()/(C+1e-10)<.03))).astype(float)+
        F('BB_Squeeze_End_Bull').astype(float)+F('BB_Squeeze_End_Bear').astype(float)
    )
    conviction=np.clip(n_evidence.values*20 + 15, 10, 90)

    return norm_score, pd.Series(conviction, index=idx)


def _committee_leading(df, N):
    """⏳ 선행 위원회 — UTBot, Hull, SqMom, 가속도, 셋업압력, CS"""
    idx=df.index
    score=pd.Series(0., index=idx)
    ...
    # Squeeze Momentum
    sq_mom=N('Squeeze_Momentum')
    sq_rising=df.get('Squeeze_Mom_Rising',pd.Series(False,index=idx)).fillna(False)
    score+=np.where(sq_mom>0, 10, np.where(sq_mom<0, -10, 0))
    score+=np.where((sq_mom>0)&sq_rising, 5, np.where((sq_mom<0)&~sq_rising, -5, 0))

    # 복합 가속도 (방향 전환 조기 감지)
    ca=N('Composite_Accel')
    score+=np.clip(ca*8, -20, 20)
    ...
    # VuManChu (강한 합류 신호)
    score+=np.where(F('VuManChu_Bull'), 20, 0)
    score-=np.where(F('VuManChu_Bear'), 20, 0)
    ...
    # 확신도: 선행지표 일치도
    agree_count=(
        (ut_dir==1).astype(float) + hma_r.astype(float) +
        (sq_mom>0).astype(float) + (ca>0).astype(float)
    )
    disagree_count=(
        (ut_dir==-1).astype(float) + (~hma_r).astype(float) +
        (sq_mom<0).astype(float) + (ca<0).astype(float)
    )
    max_agree=np.maximum(agree_count.values, disagree_count.values)
    conviction=np.clip(max_agree*20 + 10, 10, 90)
...
def _build_candle_hover(dc):
    """캔들 호버 — ★ 모든 시그널 생략 없이 표시"""
    n = len(dc)
    # 사전 계산: 바별 시그널 리스트 구축 (O(Signals) 1회)
    bar_buy_sigs = [[] for _ in range(n)]
    bar_sell_sigs = [[] for _ in range(n)]
    bar_neutral_sigs = [[] for _ in range(n)]
    bar_cs_sigs = [[] for _ in range(n)]
    ...
    for sn, cfg in SIGNAL_REGISTRY.items():
        ...
        desc = cfg['desc']
        is_strong_buy = sn in STRONG_BUY_SIGS
        is_strong_sell = sn in STRONG_SELL_SIGS
        for i in indices:
            entry = (lbl, desc)
            if cfg['dir'] == 'buy':
                bar_buy_sigs[i].append(entry)
                if is_strong_buy: bar_strong_buy[i].append(entry)
            elif cfg['dir'] == 'sell':
                bar_sell_sigs[i].append(entry)
                if is_strong_sell: bar_strong_sell[i].append(entry)
            else:
                bar_neutral_sigs[i].append(entry)
    ...
    for cn, ccfg in COMBINED_SCAN_REGISTRY.items():
        ...
        if not vals.any(): continue
        indices = np.where(vals)[0]
        lbl = f"{ccfg['icon']}{ccfg['kor']}"
        desc = ccfg['desc']
        is_sb = cn in STRONG_BUY_SIGS
        is_ss = cn in STRONG_SELL_SIGS
        for i in indices:
            bar_cs_sigs[i].append((lbl, desc, ccfg['dir']))
    ...
    es_v = dc.get('Ensemble_Score', pd.Series(0, index=dc.index)).values
    ...
    veto_v = dc.get('Veto_Flags', pd.Series('', index=dc.index)).values

    # 위원회 데이터
    cm_scores = {}; cm_votes = {}
    for cm in COMMITTEE_NAMES:
        cm_scores[cm] = dc.get(f'CM_{cm}_EffScore', pd.Series(0, index=dc.index)).values
        v = dc.get(f'CM_{cm}_Vote', pd.Series(0, index=dc.index)).values
        cm_votes[cm] = v

    texts = []
    for i in range(n):
        net = bt_v[i] - st_v[i]
        ctx_label = CTX_KOR.get(int(ctx_v[i]), '기본')
        lines = [
            f"<b>{dates[i]}</b> O:{ov[i]:.2f} H:{hv[i]:.2f} L:{lv[i]:.2f} C:{cv[i]:.2f}",
            f"Vol:{vol_v[i]:,.0f} ATR:{atr_v[i]:.2f}",
            f"─" * 28,
            f"<b>📍{jg_v[i]}</b>({cf_v[i]:.0f}%) ES:{es_v[i]:+.1f} [{ctx_label}]",
        ...
        vote_parts = []
        for cm in COMMITTEE_NAMES:
            v = cm_votes[cm][i]; s = cm_scores[cm][i]
            icon = COMMITTEE_ICONS.get(cm, '•')
            if v == 1: vote_parts.append(f"<span style='color:#6EE7B7'>{icon}B{s:+.0f}</span>")
            elif v == -1: vote_parts.append(f"<span style='color:#FCA5A5'>{icon}S{s:+.0f}</span>")
            elif v == -99: vote_parts.append(f"<span style='color:#475569'>{icon}—</span>")
            else: vote_parts.append(f"<span style='color:#FCD34D'>{icon}N{s:+.0f}</span>")
        lines.append(' '.join(vote_parts))

        # Veto 표시
        veto_str = str(veto_v[i]) if veto_v[i] else ''
        if veto_str:
            lines.append(f"<span style='color:#FCA5A5'>🚫Veto: {veto_str}</span>")
        ...
        if bar_strong_buy[i] or bar_strong_sell[i]:
            lines.append(f"─" * 28)
            for lbl, desc in bar_strong_buy[i]:
                lines.append(f"<span style='color:#6EE7B7'>⭐{lbl}: {desc}</span>")
            for lbl, desc in bar_strong_sell[i]:
                lines.append(f"<span style='color:#FCA5A5'>⭐{lbl}: {desc}</span>")

        # Combined Scan (전부 표시)
        if bar_cs_sigs[i]:
            lines.append(f"─" * 28)
            for lbl, desc, d in bar_cs_sigs[i]:
                clr = '#6EE7B7' if d == 'buy' else ('#FCA5A5' if d == 'sell' else '#FCD34D')
                lines.append(f"<span style='color:{clr}'>🎯{lbl}: {desc}</span>")

        # ★★★ 매수/매도/중립 시그널 — 생략 없이 모두 표시 ★★★
        bs = bar_buy_sigs[i]; ss = bar_sell_sigs[i]; ns = bar_neutral_sigs[i]
        if bs or ss or ns:
            lines.append(f"─" * 28)
        if bs:
            lines.append(f"<span style='color:#6EE7B7'><b>▲매수({len(bs)})</b></span>")
            for lbl, desc in bs:  # ★ 제한 없음!
        ...
        if ss:
            ...
            for lbl, desc in ss:  # ★ 제한 없음!
                lines.append(f"<span style='color:#FCA5A5'> {lbl} <span style='color:#94A3B8;font-size:10px'>({desc})</span></span>")
        if ns:
            lines.append(f"<span style='color:#FCD34D'><b>◆중립({len(ns)})</b></span>")
            for lbl, desc in ns:  # ★ 제한 없음!
                lines.append(f"<span style='color:#FCD34D'> {lbl} <span style='color:#94A3B8;font-size:10px'>({desc})</span></span>")
...
# ══════════════════════════════════════════════════════════════
#  build_chart — 8-Row + Ensemble Score Row
# ══════════════════════════════════════════════════════════════
...
def build_chart(dc, ticker):
    """8-Row 차트: 캔들+Vol+WT+MACD+MFI+StochSlow+SqMom+Ensemble"""
    ...
    fig = make_subplots(rows=8, cols=1, shared_xaxes=True, vertical_spacing=0.02,
        ...
        subplot_titles=(ticker,"Vol","WaveTrend","MACD","Money Flow (MFI)","Stoch Slow","Squeeze Mom","5-Committee Ensemble"))

    # ═══ Row 1: 캔들 + MA + BB + ST + HMA + UTBot ═══
    ...
    if sb.any():
        ...
        hover_texts = []
        for bar_idx in sr.index:
            bar_reasons = []
            for sn in STRONG_BUY_SIGS:
                if sn in dc.columns and dc.loc[bar_idx, sn]:
                    reg = SIGNAL_REGISTRY.get(sn) or COMBINED_SCAN_REGISTRY.get(sn, {})
                    bar_reasons.append(reg.get('kor', sn))
            reason_str = ', '.join(bar_reasons[:5]) if bar_reasons else '다중 강세 합류'
            hover_texts.append(f"<b>⭐ 강력매수</b><br><span style='color:#94A3B8'>{reason_str}</span><br>{bar_idx.strftime('%Y-%m-%d')}")
        fig.add_trace(go.Scatter(x=sr.index, y=yv, mode='markers',
            marker=dict(symbol='star', size=8, color='#FFD700', line=dict(width=2, color='#00E676'), opacity=.95),
            name='⭐강력매수', hovertemplate="%{text}<extra></extra>", text=hover_texts), row=1, col=1)
    if ss.any():
        ...
        for bar_idx in sr.index:
            ...
            for sn in STRONG_SELL_SIGS:
                if sn in dc.columns and dc.loc[bar_idx, sn]:
                    reg = SIGNAL_REGISTRY.get(sn) or COMBINED_SCAN_REGISTRY.get(sn, {})
                    bar_reasons.append(reg.get('kor', sn))
            reason_str = ', '.join(bar_reasons[:5]) if bar_reasons else '다중 약세 합류'
            hover_texts.append(f"<b>⭐ 강력매도</b><br><span style='color:#94A3B8'>{reason_str}</span><br>{bar_idx.strftime('%Y-%m-%d')}")
        fig.add_trace(go.Scatter(x=sr.index, y=yv, mode='markers',
            marker=dict(symbol='star', size=8, color='#FFD700', line=dict(width=2, color='#FF1744'), opacity=.95),
            name='⭐강력매도', hovertemplate="%{text}<extra></extra>", text=hover_texts), row=1, col=1)
    ...
    # ═══ Row 5: Money Flow (MFI 센터링) ═══
    mfi_raw = dc.get('MFI', pd.Series(50, index=dc.index))
    mfi_centered = mfi_raw - 50
    ...
    fig.add_trace(go.Scatter(x=dc.index, y=mfi_centered,
        ...
        customdata=mfi_raw.values), row=5, col=1)
    ...
    fig.add_hline(y=30, line_dash='dash', line_color='#FF5252', line_width=1, row=5, col=1)
    fig.add_hline(y=-30, line_dash='dash', line_color='#4FC3F7', line_width=1, row=5, col=1)
    fig.add_hline(y=0, line_dash='solid', line_color='#475569', line_width=1.5, row=5, col=1)
    mfi_for_marker = mfi_centered
    ...
    fig.add_trace(go.Bar(x=dc.index, y=sq_mom, marker_color=sq_c.tolist(),
        name="SqMom", opacity=.85, hovertemplate="SqMom:%{y:.3f}<extra></extra>"), row=7, col=1)
    ...
    # ═══ Row 8: 5-Committee Ensemble Score ═══
    if 'Ensemble_Score' in dc.columns:
        es = dc['Ensemble_Score']
        colors = np.where(es >= 30, '#00E676',
                 np.where(es >= 10, '#69F0AE',
                 np.where(es <= -30, '#FF1744',
                 np.where(es <= -10, '#FF5252', '#FFC107'))))

        # 위원회별 데이터 스택
        cm_data = []
        for cm in COMMITTEE_NAMES:
            cm_data.append(dc.get(f'CM_{cm}_EffScore', pd.Series(0, index=dc.index)).values)
        ...
        fig.add_trace(go.Bar(x=dc.index, y=es, marker_color=colors.tolist(),
            name="Ensemble", opacity=.85,
            customdata=np.column_stack([jg_v, cf_v, ba_v, sa_v,
                cm_arr[:,0], cm_arr[:,1], cm_arr[:,2], cm_arr[:,3], cm_arr[:,4],
                [CTX_KOR.get(int(c),'-') for c in ctx_v]]),
            hovertemplate=(
                "<b>%{customdata[0]}</b> (%{customdata[1]:.0f}%) ES:%{y:+.1f}<br>"
                "찬성: B%{customdata[2]}개 S%{customdata[3]}개 [%{customdata[9]}]<br>"
                "📈Trend:%{customdata[4]:+.0f} 🔥Mom:%{customdata[5]:+.0f} "
                "💰Money:%{customdata[6]:+.0f}<br>"
                "🏗️Struct:%{customdata[7]:+.0f} ⏳Lead:%{customdata[8]:+.0f}"
                "<extra></extra>"
            )), row=8, col=1)
        ...
        # 컨텍스트 배경색
        ctx_colors = {
            CTX_EXTREME_OS: 'rgba(0,230,118,.06)',
            CTX_EXTREME_OB: 'rgba(255,23,68,.06)',
            CTX_ACCUMULATION: 'rgba(0,188,212,.06)',
            CTX_DISTRIBUTION: 'rgba(255,87,34,.06)',
            CTX_STRONG_UP: 'rgba(0,230,118,.03)',
            CTX_STRONG_DN: 'rgba(255,23,68,.03)',
        }
        prev_ctx = -1; seg_start = 0
        ctx_vals = dc.get('Market_Context', pd.Series(0, index=dc.index)).values
        for ci in range(len(dc)):
            cur = int(ctx_vals[ci])
            if cur != prev_ctx:
                if prev_ctx in ctx_colors and ci > seg_start:
                    fig.add_vrect(x0=dc.index[seg_start], x1=dc.index[ci-1],
                        fillcolor=ctx_colors[prev_ctx], line_width=0, row=8, col=1)
                seg_start = ci; prev_ctx = cur
        if prev_ctx in ctx_colors:
            fig.add_vrect(x0=dc.index[seg_start], x1=dc.index[-1],
                fillcolor=ctx_colors[prev_ctx], line_width=0, row=8, col=1)
...
def build_metadata(dc, ticker):
    ...
    committee = {}
    for cm in COMMITTEE_NAMES:
        vote_val = int(_sf(lat.get(f'CM_{cm}_Vote', 0)))
        committee[cm] = {
            'score': _sf(lat.get(f'CM_{cm}_EffScore', 0)),
            'conviction': _sf(lat.get(f'CM_{cm}_EffConv', 0)),
            'vote': 'BUY' if vote_val == 1 else ('SELL' if vote_val == -1 else ('ABSTAIN' if vote_val == -99 else 'NEUTRAL')),
            'vote_int': vote_val,
        }
    ...
    return {
        ...
        'ensemble_score': _sf(lat.get('Ensemble_Score')),
        ...
        'context': ctx_code,
        'context_label': CTX_KOR.get(ctx_code, '기본'),
        'committee': committee,
        'buy_agree': int(_sf(lat.get('Buy_Agree'))),
        'sell_agree': int(_sf(lat.get('Sell_Agree'))),
        'veto_flags': str(lat.get('Veto_Flags', '')),
...
def render_price_header(m):
    ...
    jg=m['judgment'];cf=m['confidence'];es=m.get('ensemble_score',0)
    ...
    st.markdown(f"""<div class="price-header"><p style="color:#64748B;font-size:.8rem;margin:0">🚦 {m['ticker']} · {m['last_date']} · <b style="color:#A5B4FC">{m['regime_label']}</b> · <span class='context-badge' style='background:rgba(99,102,241,.1);color:#A5B4FC'>🌐{ctx_l}</span></p>
...
def render_judgment_card(m):
    ...
    ba=m.get('buy_agree',0);sa=m.get('sell_agree',0)
    veto=m.get('veto_flags','')
    veto_html=f"<div style='margin-top:6px'><span class='veto-badge'>🚫 {veto}</span></div>" if veto else ""
    ctx_l=m.get('context_label','기본')
    st.markdown(f"""<div class="score-card {cc}"><p style="font-size:2rem;font-weight:800;color:{jc};margin:0">{labels.get(jg,jg)}</p>
        ...
        <div style="border-left:1px solid rgba(255,255,255,.08);padding-left:32px"><p style="color:#64748B;font-size:.7rem;margin:0">컨텍스트</p><p style="color:#A5B4FC;font-size:1rem;font-weight:700;margin:2px 0">🌐{ctx_l}</p></div></div>{veto_html}</div>""",unsafe_allow_html=True)


def render_committee_panel(m):
    """🏛️ 5-Committee 투표 상세 패널"""
    ...
    st.markdown(f"""<div style='background:rgba(99,102,241,.06);border:1px solid #6366F133;border-radius:12px;padding:12px 16px;margin-bottom:12px'>
        <span style='font-size:1.1rem;font-weight:800;color:#A5B4FC'>🌐 컨텍스트: {ctx_l}</span>
        <span style='color:#64748B;margin-left:12px;font-size:.8rem'>가중치 자동 조정됨</span></div>""", unsafe_allow_html=True)
    ...
    for ci, cm in enumerate(COMMITTEE_NAMES):
        ...
        # 투표 색상
        if vote == 'BUY':
            vote_class = 'cm-vote-buy'; vote_icon = '🟢'; bar_color = '#34D399'
        elif vote == 'SELL':
            vote_class = 'cm-vote-sell'; vote_icon = '🔴'; bar_color = '#F87171'
        elif vote == 'ABSTAIN':
            vote_class = 'cm-vote-abstain'; vote_icon = '⬜'; bar_color = '#475569'
        else:
            vote_class = 'cm-vote-neutral'; vote_icon = '🟡'; bar_color = '#FCD34D'

        # 점수 바 (양수=오른쪽 초록, 음수=왼쪽 빨강)
        bar_pct = min(abs(score), 100)
        bar_left = max(50 - bar_pct/2, 0) if score < 0 else 50
        bar_right = 50 if score < 0 else min(50 + bar_pct/2, 100)
        fill_color = '#34D399' if score > 0 else '#F87171' if score < 0 else '#475569'

        st.markdown(f"""<div class='cm-card'>
            <div style='display:flex;justify-content:space-between;align-items:center'>
                <div style='display:flex;align-items:center;gap:8px'>
                    <span style='font-size:1.2rem'>{icon}</span>
                    <span style='color:#E8ECF1;font-weight:700'>{cm}</span>
                    <span style='color:#475569;font-size:.7rem'>×{weight:.0%}</span>
                </div>
                <div style='display:flex;align-items:center;gap:12px'>
                    <span class='{vote_class}' style='font-size:.9rem'>{vote_icon} {vote}</span>
                    <span style='color:{"#34D399" if score>0 else "#F87171" if score<0 else "#FCD34D"};font-weight:800;font-size:1.1rem'>{score:+.0f}</span>
                </div>
            </div>
            <div style='display:flex;align-items:center;gap:8px;margin-top:6px'>
                <span style='color:#64748B;font-size:.7rem;width:40px'>확신도</span>
                <div style='flex:1;height:6px;background:#151921;border-radius:3px;overflow:hidden'>
                    <div style='width:{conv}%;height:6px;background:{bar_color};border-radius:3px'></div>
                </div>
                <span style='color:#94A3B8;font-size:.75rem;width:35px;text-align:right'>{conv:.0f}%</span>
            </div>
            <div style='margin-top:4px;height:4px;background:#0B0E14;border-radius:2px;position:relative'>
                <div style='position:absolute;left:50%;top:-1px;width:1px;height:6px;background:#475569'></div>
                <div style='position:absolute;left:{bar_left}%;width:{bar_right-bar_left}%;height:4px;background:{fill_color};border-radius:2px'></div>
            </div>
    ...
    veto = m.get('veto_flags', '')
    if veto:
        st.markdown(f"""<div style='background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.2);border-radius:10px;padding:10px 14px;margin-top:10px'>
            <span style='color:#FCA5A5;font-weight:700'>🚫 거부권 발동:</span>
            <span style='color:#E8ECF1;margin-left:8px'>{veto}</span></div>""", unsafe_allow_html=True)
    ...
    if m or fig_json:
        ...
        with t0:
            if fig_json:
                ...
                st.caption("💡 **캔들에 마우스를 올리면** 해당 일자의 모든 시그널·위원회 투표·판단을 볼 수 있습니다.")
        with t1:
            if m:
                render_judgment_card(m)
                st.markdown("#### 🏛️ 5-Committee 투표 상세")
                render_committee_panel(m)
        with t2:
            if m:
                st.markdown("#### 📊 10-Layer (참고)")
                render_10layer_bars(m)
        with t3:
...
def build_prompt_text(dc, meta):
    ...
    # Block D: 5-Committee Ensemble 판단 (신규)
    cm = m.get('committee', {})
    cm_lines = []
    for name in COMMITTEE_NAMES:
        ...
        cm_lines.append(
            f"    {icon}{name}: 투표={d.get('vote','?')} 점수={d.get('score',0):+.0f} 확신도={d.get('conviction',0):.0f}%"
        )
    cm_str = "\n".join(cm_lines)
    ...
    block_d = (
        ...
        f"  ──────────\n"
        f"  [참고: 10-Layer 점수]\n"
        f"  BUY: {m['buy_total']:.1f} (활성 {m['buy_active']}/10) [{buy_layer_str}]\n"
        f"  SELL: {m['sell_total']:.1f} (활성 {m['sell_active']}/10) [{sell_layer_str}]\n"
        f"  선행지표: {m['leading_verdict']}\n"
        f"  후행지표: {m['lagging_verdict']}\n"
    )
    if m['combined_scans']:
...
고객사의 퀀트 트레이딩 시스템(CipherX V14 — 5-Committee Ensemble)이 산출한 결과를 **교차 검증(Cross-Validation)**하는 것이 당신의 임무입니다.
시스템을 무비판적으로 수용하는 '예스맨'이 아니라, 시스템이 놓치고 있는 맹점과 리스크를 짚어내는 **'Devil's Advocate(악마의 변호인)'** 역할을 수행하세요.

━━━  V14 시스템 이해 ━━━
CipherX V14는 5개 독립 위원회(Trend, Momentum, Money, Structure, Leading)가 각자 판단 후 가중 투표하는 앙상블 시스템입니다.
- 각 위원회는 -100~+100 점수와 확신도(%)를 가집니다.
- "시장 컨텍스트"(극과매도/매집/분배/강한추세 등)에 따라 위원 가중치가 동적으로 변합니다.
- "거부권(Veto)"이 있어 극단 상황에서 특정 위원의 판단을 무력화합니다.
- Block D의 위원회 투표를 반드시 분석하세요. 어느 위원이 소수 의견인지, 그 이유가 타당한지 검증하세요.

━━━  Rules (핵심 원칙) ━━━
1. **데이터 위계**: Block A~C(원시 데이터)를 먼저 독자적으로 분석한 뒤, Block D(시스템 판단)와 대조하세요.
2. **반드시 반론 제시**: 시스템이 BUY라면 SELL 근거를 적극 탐색하고, SELL이라면 BUY 근거를 찾으세요.
3. **위원회 분석**: 소수 의견 위원의 논리가 맞을 가능성을 반드시 검토하세요. "Momentum이 BUY인데 Trend가 SELL이면, 이것이 바닥 반전 신호인지 아니면 하락 추세 반등인지?"
4. **Veto 검증**: 거부권 발동 여부와 그 타당성을 평가하세요. 발동되지 않았지만 발동되어야 할 상황은 없는지?
5. **확률적 사고**: 시나리오별 확률(%)과 조건부 트리거를 명시하세요.
6. **구체적 가격**: 지지/저항은 VP_POC, VAH, VAL, MA, ATR 기반으로 소수점 둘째 자리까지 명시하세요.
7. **자금 추적**: CMF, OBV, 거래량 비율로 스마트 머니의 매집/분배/관망 상태를 추론하세요.
8. **환각 금지**: 데이터에 없는 외부 정보를 지어내지 마세요.
...
###  2. 위원회 교차 검증 (V14 핵심)
* **컨텍스트 평가:** [시스템이 감지한 컨텍스트({m.get('context_label','')})가 적절한지?]
* **다수 의견 검증:** [다수 위원의 판단이 원시 지표와 일치하는지?]
* **소수 의견 분석:** [소수 의견 위원의 논리가 오히려 맞을 가능성은?]
* **Veto 평가:** [거부권 발동 여부의 타당성. 미발동이지만 필요했던 경우?]
* **❌ 시스템 맹점:** [위원회가 놓치고 있는 리스크]

###  3. 핵심 시그널 해석
* **WaveTrend/VuManChu:** [과매수·과매도, 다이버전스, 교차 해석]
...
※ 위원회 투표에서 소수 의견이 맞을 가능성에 대한 최종 견해를 반드시 포함하세요."""
...
if current_mode == '스캐너':
    ...
    if ci and ci.strip():
        ...
        tickers = [t.strip().upper() for t in ci.split(',') if t.strip()] if ci else []
        scan_source = "직접 입력"
    ...
    if scan_btn and tickers:
        ...
        pb.progress(1.0, text=f"✅ {len(results)}/{len(tickers)} 완료")
        time.sleep(.3); pb.empty()

        # 정렬: 시그널 유무 → 최신 시그널 → T1 개수
        from datetime import datetime as dt_
        def _sort_key(x):
            has_scans = 0 if x['scans'] else 1
            try:
                ts_val = -dt_.strptime(x['latest_sig'], '%Y-%m-%d').timestamp() if x['latest_sig'] != '9999-99-99' else 0
            except: ts_val = 0
            t1_count = -sum(1 for s in x['scans'] if s['tier'] == 1)
            return (has_scans, ts_val, t1_count)
    ...
    if not results:
        if scan_source_display:
            st.info(f"🔍 {scan_source_display}에서 결과 없음")
    else:
        ...
        stats_html = (
            ...
            f"<span style='color:#64748B;font-size:.8rem;margin-left:4px'>매수 판단</span></div>"
            ...
            f"<span style='color:#64748B;font-size:.8rem;margin-left:4px'>매도 판단</span></div>"
        ...
        for r in results:
            ...
            # 앙상블 점수 + 컨텍스트 표시
            es_color = '#34D399' if r['es'] > 10 else ('#F87171' if r['es'] < -10 else '#FCD34D')
            ensemble_info = f"<span style='color:{es_color};font-size:.75rem;font-weight:700'>ES:{r['es']:+.0f}</span>"
            ctx_info = f"<span style='color:#A5B4FC;font-size:.65rem'>{r.get('ctx','')}</span>"
            agree_info = f"<span style='color:#64748B;font-size:.65rem'>B{r.get('ba',0)}:S{r.get('sa',0)}</span>"
            ...
            card_html = (
                ...
                f"{ensemble_info} {agree_info} {ctx_info}"
    ...
    for i, msg in enumerate(st.session_state.messages):
        ...
        with st.chat_message(msg["role"], avatar=av):
            if msg.get("type") == "analysis":
                ...
                st.download_button("", key=f"dl_{i}",
                    data=msg["content"].encode('utf-8'),
                    file_name=f"{msg.get('ticker','')}_V14_{datetime.now().strftime('%Y%m%d')}.md",
        ...
        with st.chat_message("assistant", avatar="✨"):
            ...
            try:
                ...
                st.session_state.messages.append({
                    "role":"assistant","type":"report","ticker":tp.upper(),"content":"".join(col_)
                })
                st.session_state.pending_ai_ticker = None
                st.session_state.pending_ai_prompt = None
                st.rerun()
            except Exception as e:
                pb.empty(); st.error(f"AI오류:{e}")
    ...
    def process_ticker(tv, refresh=False):
        ...
        with st.chat_message("assistant", avatar="✨"):
            with st.status(f" {tv} 분석중...", expanded=True) as status:
                ...
                st.write(" 139시그널+32CS+5-Committee Ensemble 분석...")
                fig_json, phist, meta = analyze(tv, chart_days, refresh)
                if fig_json and meta:
                    cs_n = len(meta.get('combined_scans', []))
                    t1_n = sum(1 for s in meta.get('combined_scans', []) if s['tier'] == 1)
                    es = meta.get('ensemble_score', 0)
                    ctx_l = meta.get('context_label', '기본')
                    ba = meta.get('buy_agree', 0); sa = meta.get('sell_agree', 0)
                    veto = meta.get('veto_flags', '')
                    st.write(f" 판단:{meta['judgment']}({meta['confidence']:.0f}%) ES:{es:+.1f} [{ctx_l}]")
                    st.write(f"🏛️ 투표: B{ba}:S{sa} | CS:{cs_n}(T1:{t1_n})")
            ...
            if fig_json:
                ...
                ctx_l = meta.get('context_label', '기본')
                ba = meta.get('buy_agree', 0); sa = meta.get('sell_agree', 0)
                content += f"\n🏛️ Ensemble: {es:+.1f} | 투표: B{ba}:S{sa} | 🌐{ctx_l}"
                ...
                st.session_state.messages.append({
                    "role":"assistant","type":"text","content":f"⚠️ **{tv}** 실패:{phist}"
                })
                st.rerun()
