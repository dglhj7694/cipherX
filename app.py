...
def analyze(ticker, chart_days=252, refresh=False, show_trendlines=True):
    try:
        ...
        return build_chart(dc, ticker, show_trendlines=show_trendlines).to_json(), build_prompt_text(dc, meta), meta
...
with st.sidebar:
    ...
    show_trendlines = st.checkbox("자동 빗각 표시", value=True, key="show_trendlines")
    ...
    def process_ticker(tv, refresh=False):
        ...
        with st.chat_message("assistant", avatar="✨"):
            with st.status(f"🔍 {tv} 분석 중...", expanded=True) as status:
                ...
                fj, phist, meta = analyze(tv, chart_days, refresh, show_trendlines=show_trendlines)
                if fj and meta:
                    jg  = meta['judgment']
