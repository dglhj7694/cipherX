# ══════════════════════════════════════════════════════════════
#  CipherX V11.0 — Judgment-First Architecture (최종 최적화본)
# ══════════════════════════════════════════════════════════════
...
try:
    ...
    def render_company_details(ticker): pass
...
def fetch_fundamentals(ticker):
    try:
        ...
        logging.warning(f"Fetch fundamentals failed for {ticker}: {e}")
...
def _detect_engulfing_pair(c, o, wt1, wt_t=20):
    ...
    bull = (prev_bearish & (c > o) & (curr_body_low <= prev_body_low) & (curr_body_high >= prev_body_high) & big & (wt1 < -wt_t))
    bear = (prev_bullish & (c < o) & (curr_body_low <= prev_body_low) & (curr_body_high >= prev_body_high) & big & (wt1 > wt_t))
    return bull, bear
...
def detect_candlestick_patterns(c, o, h, l, wt1, atr):
    ...
    min_range = atr * 0.5
    hammer = ((lower_shadow >= body * 2) & (upper_shadow <= body * 0.3) & is_small & (wt1 < -20) & (c >= o) & (full_range > min_range))
    shooting = ((upper_shadow >= body * 2) & (lower_shadow <= body * 0.3) & is_small & (wt1 > 20) & (c <= o) & (full_range > min_range))
    ...
    spin = (is_small & (upper_shadow > body * 0.5) & (lower_shadow > body * 0.5) & (spin_ratio > 0.5) & (spin_ratio < 2.0) & ~doji)
...
def compute_trade_judgment(df):
    ...
    # 🔧 초단기 EMA 스무딩 (반응속도 유지 + 휩소 제거)
    macd_h = df['MACD_Hist']
    macd_h_ema = macd_h.ewm(span=3, adjust=False).mean()
    macd_h_rising = macd_h_ema > macd_h_ema.shift(1)
    macd_h_falling = macd_h_ema < macd_h_ema.shift(1)
    ...
    bm += np.where(df['RSI'] > 80, -1.0,
          np.where(df['RSI'] >= 60, 2.0,
          np.where(df['RSI'] >= 50, 1.0, 0)))
    bm += np.where(df['StochK'] > 85, -1.0,
          np.where(df['StochK'] >= 60, 2.0,
          np.where(df['StochK'] >= 50, 1.0, 0)))
    bm += np.where(df['WT1'] > 60, -1.0,
          np.where(df['WT1'] >= 20, 2.0,
    ...
    be_pts = np.where(be_raw & in_downtrend & (body_atr_ratio > 1.0), 3.5,
             np.where(be_raw & in_downtrend, 3.0,
             np.where(be_raw & (body_atr_ratio > 1.0), 2.5,
    ...
    pct_b = df['Percent_B']
    bb += np.where(pct_b < 0.05, 2.5,
          np.where(pct_b < 0.2, 1.5,
          np.where((pct_b >= 0.4) & (pct_b <= 0.6) & above_50, 0.5,
          np.where(pct_b > 0.95, -1.5, 0))))

    blb = df.get('Below_Lower_BB', pd.Series(False, index=idx)).fillna(False)
    bb += np.where(blb & above_200, 2.0,
          np.where(blb & below_200, -0.5,
          np.where(blb, 1.0, 0)))
    ...
    bv += np.where(vol_bull_ratio > 2.0, 1.5,
          np.where(vol_bull_ratio > 1.3, 0.5,
          np.where(vol_bull_ratio < 0.5, -1.0, 0)))
    ...
    bmf += np.where((rmfi < -10) & (mf_slope > 0), 2.5,
           np.where((rmfi < -10) & (mf_slope <= 0), 0.0,
           np.where((rmfi < -5) & (mf_slope > 0), 1.0,
           np.where(rmfi > 10, -1.0, 0))))
    ...
    sm += np.where(df['RSI'] < 20, -1.0,
          np.where(df['RSI'] <= 40, 2.0,
          np.where(df['RSI'] <= 50, 1.0, 0)))
    sm += np.where(df['StochK'] < 15, -1.0,
          np.where(df['StochK'] <= 40, 2.0,
          np.where(df['StochK'] <= 50, 1.0, 0)))
    sm += np.where(df['WT1'] < -60, -1.0,
          np.where(df['WT1'] <= -20, 2.0,
          np.where(df['WT1'] <= 0, 1.0, 0)))
    ...
    smf += np.where((rmfi > 10) & (mf_slope < 0), 2.5,
           np.where((rmfi > 10) & (mf_slope >= 0), 0.0,
           np.where((rmfi > 5) & (mf_slope < 0), 1.0,
           np.where(rmfi < -10, -1.0, 0))))
    ...
    for i in range(len(df)):
        ...
        if b >= 18 and bal >= 4 and ratio >= 2.5 and diff >= 12:
            j[i] = 'STRONG_BUY' if combo_b else 'WATCH_BUY'
        elif b >= 13 and bal >= 4 and ratio >= 1.5 and diff >= 7:
            j[i] = 'BUY' if combo_b else 'NEUTRAL'
        elif b >= 8 and bal >= 3 and diff >= 4:
            j[i] = 'WATCH_BUY'
        elif s >= 18 and sal >= 4 and s_ratio >= 2.5 and (s-b) >= 12:
            j[i] = 'STRONG_SELL' if combo_s else 'WATCH_SELL'
        elif s >= 13 and sal >= 4 and s_ratio >= 1.5 and (s-b) >= 7:
            j[i] = 'SELL' if combo_s else 'NEUTRAL'
        elif s >= 8 and sal >= 3 and (s-b) >= 4:
            j[i] = 'WATCH_SELL'
...
def get_judgment_detail(row):
    ...
    rationale = get_trade_rationale(judgment, combos, bl, sl)
    return {
        'judgment': judgment,
...
def detect_mf_signals(c, rmfi):
    ...
    for i in range(1, len(c)):
        if mf_rising.iloc[i]: mf_up_streak.iloc[i] = mf_up_streak.iloc[i-1] + 1
        else: mf_up_streak.iloc[i] = 0
        if mf_falling.iloc[i]: mf_dn_streak.iloc[i] = mf_dn_streak.iloc[i-1] + 1
        else: mf_dn_streak.iloc[i] = 0
...
def build_chart(dc, ticker, regime, shield):
    ...
    if 'Trade_Judgment' in dc.columns:
        ...
        for j_grade, j_cfg in JUDGMENT_MARKERS.items():
            ...
            sig_rows = dc[mask]
            if j_cfg['base'] == 'Low': y_vals = sig_rows['Low'] + sig_rows['ATR'] * j_cfg['atr_mult']
            else: y_vals = sig_rows['High'] + sig_rows['ATR'] * j_cfg['atr_mult']
...
def build_prompt_text(dc, meta):
    ...
    if jd:
        ...
        j_txt += f"  💡 핵심 판단 이유: {jd.get('rationale', '없음')}\n"
...
5. 🎯 멀티 시그널 판단: [멀티 시그널 매매 판단] 섹션의 7-Layer 점수, 콤보, 최종 판단, **특히 '핵심 판단 이유'를 핵심 근거로 활용하세요.**
...
* 💡 핵심 근거: [프롬프트에 주어진 '핵심 판단 이유'를 바탕으로 설명]
...
for i, msg in enumerate(st.session_state.messages):
    ...
    with st.chat_message(msg["role"], avatar=av):
        if msg.get("type") == "analysis":
            ...
            st.download_button("📥 마크다운 파일 다운로드", key=f"dl_{i}",
