...
QBS_CHASE_WATCH_LIMIT = 20
QBS_PULLBACK_WAIT_LIMIT = 20
...
def _score_candidate(ticker: str, row: Mapping[str, Any], membership: set[str], target_date: date) -> FinalBuyCandidate:
    ...
    return FinalBuyCandidate(
        ...
        source_flags={
            ...
            "hard_exclude": hard_exclude,
            "today_chg_pct": _optional_float(row, "chg"),
            "chg_5d": _optional_float(row, "chg_5d"),
            "ret_1m_pct": _optional_float(row, "ret20_pct"),
            "ret_1y_pct": _optional_float(row, "ret252_pct"),
            "rsi": _optional_float(row, "rsi") if _optional_float(row, "rsi") is not None else _optional_float(row, "RSI"),
            "ma20_dist_pct": _optional_float(row, "dist_sma20_pct"),
            "high_pos_pct": _optional_float(row, "drawdown_from_52w_high_pct"),
        },
        membership_count=len(membership),
    )
