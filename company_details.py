...
def _metric_row(label, value, value_class="metric-value"):
...
.s-title .s-num{
    background:#21262d;border-radius:8px;padding:4px 10px;
...
/* ── 옵션 박스 ── */
.opt-box{background:rgba(0,0,0,.25);padding:14px;border-radius:12px;
...
def render_company_details(ticker_str: str):
    ...
    all_verdicts.append(("성장 사이클", v1_c))
    ...
    <div class="s-card">
        ...
        <div class="two-col">
            <div>
                {_metric_row("YOY 매출 성장률", _fmt_pct(info.get('revenueGrowth')))}
    ...
    <div class="s-card">
        ...
        <div class="two-col">
            <div>
                {_metric_row(f"{yr_rev}년 매출 CAGR" if yr_rev else "매출 CAGR", _fmt_pct(cagr_rev), cr_cls(cagr_rev))}
                {_metric_row(f"{yr_ni}년 순이익 CAGR" if yr_ni else "순이익 CAGR", _fmt_pct(cagr_ni), cr_cls(cagr_ni))}
            </div>
            <div>
                {_metric_row(f"{yr_eps}년 EPS CAGR" if yr_eps else "EPS CAGR", _fmt_pct(cagr_eps), cr_cls(cagr_eps))}
            </div>
        </div>
    ...
    all_verdicts.append(("과거 성적", v3_c))
    ...
    fut_roe   = roe_v * retention if roe_v else None
    ...
    gs = sum(1 for x in [eg and eg > 0.15, rg and rg > 0.10, retention > 0.60,
                          fut_roe and fut_roe > 0.10, peg and 0 < peg < 1.5] if x)
    ...
    <div class="s-card">
        ...
        <div class="two-col">
            <div>
                {_metric_row("이익 성장률 (YOY)", _fmt_pct(eg), eg_cls)}
                {_metric_row("매출 성장률 (YOY)", _fmt_pct(rg), rg_cls)}
            ...
            <div>
                ...
                {_metric_row("수익 유보율 (저축률)", _fmt_pct(retention))}
                {_metric_row("미래 예상 ROE (ROE×유보율)", _fmt_pct(fut_roe),
                             "m-green" if fut_roe and fut_roe > 0.1 else "m-value")}
                <div class="divider"></div>
                <div style="font-size:.78rem;color:#6e7681;line-height:1.6">
                    💡 유보율이 높을수록 성장에 재투자.<br>
                    PEG &lt; 1.5 면 성장 대비 저평가 가능성.
                </div>
            </div>
        </div>
    ...
    # 자산 구조 비율 바
    if ta and ta > 0:
        eq_pct = max(0, eq / ta * 100) if eq else 0
        li_pct = max(0, tl / ta * 100)
    else:
        eq_pct, li_pct = 50, 50
    ...
    <div class="s-card">
        ...
        <div class="two-col">
            <div>
                ...
                {_metric_row("단기 부채", _fmt_num(curr_d))}
                {_metric_row("장기 부채", _fmt_num(lt_d))}
                <div class="divider"></div>
            ...
            <div>
                ...
                {_metric_row("자본 (Equity)", _fmt_num(eq))}
                {_metric_row("순 자산", _fmt_num(na), "m-green" if na > 0 else "m-red")}
                <div class="divider"></div>
                <div style="font-size:.82rem;color:#6e7681;margin-bottom:6px;font-weight:600">자산 구조</div>
                <div class="p-container">
                    <div class="p-bar" style="width:{eq_pct:.1f}%;background:#2196F3">
                        {'자본 ' + str(round(eq_pct)) + '%' if eq_pct > 15 else ''}</div>
                    <div class="p-bar" style="width:{li_pct:.1f}%;background:#FF5722">
                        {'부채 ' + str(round(li_pct)) + '%' if li_pct > 15 else ''}</div>
                </div>
            </div>
        </div>
    ...
    if price and w52h and w52l and w52h != w52l:
        ...
        pos_bar = (
            ...
            f'<div style="display:flex;justify-content:space-between;font-size:.73rem;color:#6e7681;margin-bottom:4px">'
            f'<span>저 ${w52l:,.2f}</span><span style="color:#82aaff;font-weight:600">현재 ${price:,.2f}</span><span>고 ${w52h:,.2f}</span></div>'
            f'<div style="background:#21262d;border-radius:6px;height:12px;position:relative">'
            f'<div style="background:linear-gradient(90deg,#FF1744 0%,#FFC107 50%,#00E676 100%);width:100%;height:100%;border-radius:6px;opacity:0.25"></div>'
            f'<div style="position:absolute;top:-3px;left:{pos52:.1f}%;width:18px;height:18px;background:#82aaff;border-radius:50%;transform:translateX(-50%);border:2px solid #fff;box-shadow:0 0 8px #82aaff88"></div>'
            f'</div></div>')
    else:
        pos_str, pos_bar = "N/A", ""
    ...
    <div class="s-card">
        ...
        <div class="two-col">
            ...
            <div>
                ...
                {pos_bar}
                <div style="font-size:.78rem;color:#6e7681;margin-top:12px;line-height:1.6">
                    💡 베타 &lt; 1 = 시장 대비 안정적<br>
                    베타 &gt; 1 = 시장보다 변동성 큼
                </div>
            </div>
    ...
    if t_pe and s_pe:
        ...
        pe_visual = (
            ...
            f'<div style="font-size:.78rem;color:#8b949e;margin-bottom:6px">P/E 비교</div>'
            ...
            f'display:flex;align-items:center;justify-content:flex-end;padding-right:6px;font-size:.7rem;color:#fff;font-weight:600">{t_pe:.1f}</div></div></div>'
            f'<div style="display:flex;align-items:center;gap:8px;margin:4px 0">'
            f'<span style="min-width:50px;font-size:.75rem;color:#607D8B">섹터 평균</span>'
            f'<div style="flex:1;background:#21262d;border-radius:4px;height:18px;overflow:hidden">'
            f'<div style="width:{s_pe/max_pe*100:.1f}%;height:100%;background:#607D8B;border-radius:4px;'
            f'display:flex;align-items:center;justify-content:flex-end;padding-right:6px;font-size:.7rem;color:#fff;font-weight:600">{s_pe:.1f}</div></div></div>'
            f'</div>')
    ...
    <div class="s-card">
        ...
        <div class="two-col">
            <div>
                ...
                <div class="divider"></div>
                {_metric_row(f"섹터 ({_esc(sector)})", f"평균 P/E ≈ {s_pe}" if s_pe else "N/A")}
                {_metric_row("섹터 대비", pe_comp if pe_comp else "N/A")}
            </div>
            <div>
                {pe_visual}
                <div style="font-size:.73rem;color:#6e7681;margin-top:10px;line-height:1.5">
                    ※ 섹터 P/E는 2024~2025 기준 추정치이며 실시간이 아닙니다.
                </div>
            </div>
        </div>
    ...
    if t_low and t_high and t_median and price and t_high > t_low:
        ...
        target_bar = (
            f'<div style="margin:14px 0">'
            f'<div style="font-size:.78rem;color:#6e7681;margin-bottom:6px">목표가 범위</div>'
            f'<div style="display:flex;justify-content:space-between;font-size:.7rem;color:#6e7681;margin-bottom:3px">'
            f'<span>${t_low:,.0f}</span><span>${t_high:,.0f}</span></div>'
            f'<div style="background:#21262d;border-radius:6px;height:14px;position:relative">'
            f'<div style="position:absolute;top:-2px;left:{curr_pos:.1f}%;width:18px;height:18px;background:#FF9800;border-radius:50%;transform:translateX(-50%);border:2px solid #fff;z-index:2" title="현재가"></div>'
            f'<div style="position:absolute;top:-1px;left:{med_pos:.1f}%;width:14px;height:16px;background:#00E676;border-radius:3px;transform:translateX(-50%);z-index:1" title="중앙 목표가"></div>'
            f'</div>'
            f'<div style="display:flex;justify-content:center;gap:16px;margin-top:6px;font-size:.7rem">'
            f'<span style="color:#FF9800">● 현재가</span><span style="color:#00E676">● 중앙 목표가</span></div>'
            f'</div>')
            ...
            <div>
                ...
                <div style="text-align:center;margin-top:16px;padding:14px;
                    ...
                    <div style="font-size:1.5rem;font-weight:800;
                                color:{'#00E676' if up_pct > 0 else '#FF1744'};margin-top:4px">
                        {up_str}</div>
                </div>
            </div>
        </div>
    ...
    <div class="s-card">
        ...
        <div class="two-col">
            <div>
                ...
                <div class="divider"></div>
                {_metric_row("🏛️ 기관 투자자", _fmt_pct(inst))}
                {_metric_row("👔 내부자 / 임원", _fmt_pct(ins))}
                {_metric_row("👤 일반 / 개인 (추정)", _fmt_pct(pub))}
            </div>
            <div>
                ...
                <div class="p-container" style="height:36px;border-radius:12px">
                    <div class="p-bar" style="width:{inst*100:.1f}%;background:linear-gradient(135deg,#1565C0,#42A5F5)">
                        {'🏛️ ' + str(round(inst*100)) + '%' if inst > 0.08 else ''}</div>
                    <div class="p-bar" style="width:{ins*100:.1f}%;background:linear-gradient(135deg,#E65100,#FF9800)">
                        {'👔 ' + str(round(ins*100)) + '%' if ins > 0.04 else ''}</div>
                    <div class="p-bar" style="width:{pub*100:.1f}%;background:linear-gradient(135deg,#2E7D32,#66BB6A)">
                        {'👤 ' + str(round(pub*100)) + '%' if pub > 0.08 else ''}</div>
                </div>
                <div style="display:flex;gap:14px;margin-top:8px;font-size:.73rem;color:#8b949e;flex-wrap:wrap">
                    <span>🟦 기관</span><span>🟧 내부자</span><span>🟩 일반</span>
                </div>
            </div>
        </div>
    ...
    if mp and price and price > 0:
        ...
        if d > 2:
            mp_note = f"현재가 대비 {d:+.1f}% 위 → 상승 압력 가능"
            mp_c = "green"
        elif d < -2:
            mp_note = f"현재가 대비 {d:+.1f}% 아래 → 하락 압력 가능"
            mp_c = "red"
        else:
    ...
    ch = "".join([f"<li>${c_i['strike']:.1f} <span style='color:#6e7681'>(Vol {int(c_i['volume']):,})</span></li>"
                  for c_i in (tc or [])]) or "<li>N/A</li>"
    ph = "".join([f"<li>${p_i['strike']:.1f} <span style='color:#6e7681'>(Vol {int(p_i['volume']):,})</span></li>"
                  for p_i in (tp or [])]) or "<li>N/A</li>"
    ...
    sp_gauge = _gauge_bar(min(sp_pct * 2, 100), sp_color, 12)  # 50% = full bar
    ...
    <div class="s-card">
        ...
        <div class="two-col">
            <div>
                ...
                <div style="font-size:.78rem;color:#6e7681;margin-bottom:4px">공매도 게이지 (50%=가득 참)</div>
                {sp_gauge}
        ...
        if news_list and len(news_list) > 0:
            ...
            for n in news_list[:8]:
                title = (n.get('title') or
                         n.get('content', {}).get('title', '제목 없음'))
                link  = (n.get('link') or
                         n.get('content', {}).get('canonicalUrl', {}).get('url', '#'))
                pub_name = (n.get('publisher') or
