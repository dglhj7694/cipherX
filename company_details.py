...
def _calc_cagr_with_years(financials, row_candidates):
    try:
        ...
        if len(series) >= 2:
            ...
            s_val = float(series.iloc[0])
            e_val = float(series.iloc[-1])
            years = len(series) - 1
            if s_val > 0 and e_val > 0 and years > 0:
                return (e_val / s_val) ** (1.0 / years) - 1.0, years
    except Exception:
        pass
    return None, 0
...
def _annual_values(financials, row_candidates):
    try:
        ...
        if len(series) > 0:
            ...
            vals = []
            dates = []
            for idx_val, v in series.items():
                try:
                    fv = float(v)
                    if not np.isnan(fv):
                        vals.append(fv)
                        dates.append(idx_val)
                except (ValueError, TypeError):
                    pass
            return vals, dates
    except Exception:
...
def _verdict_badge(color, emoji, text):
    ...
    bc = bg.get(color, bg["gray"])
    bd = bdr.get(color, bdr["gray"])
    return (f'<div style="background:{bc};border:1px solid {bd};border-radius:10px;'
            f'padding:14px 18px;margin-top:16px;text-align:center">'
            f'<span style="font-size:1.05rem;font-weight:700;color:{bd}">{emoji} {text}</span></div>')
...
def _gauge_bar(pct, color="#00E676", height=8):
    ...
    return (f'<div style="background:#21262d;border-radius:{height}px;height:{height}px;'
...
def _chart_revenue_profit(fin, chart_key="rev_profit"):
    ...
    for i in range(n - 1, -1, -1):
        d = rd[i]
        yr = d.strftime('%Y') if hasattr(d, 'strftime') else str(d)[:4]
        years.append(yr)
        revenues.append(rv[i])
    for i in range(n - 1, -1, -1):
        incomes.append(nv[i])

    margins = []
    for ni_val, rev_val in zip(incomes, revenues):
        if rev_val and rev_val != 0:
            margins.append(ni_val / rev_val * 100)
        else:
            margins.append(0)
    ...
    try:
        ...
        fig.add_trace(go.Bar(
            ...
            text=[_fmt_num(v) for v in revenues],
            textposition="outside", yaxis="y"
        ))
        fig.add_trace(go.Bar(
            x=years, y=incomes, name="순이익",
            marker_color=["#00E676" if v >= 0 else "#FF1744" for v in incomes],
            opacity=0.85,
            text=[_fmt_num(v) for v in incomes],
            textposition="outside", yaxis="y"
        ))
        fig.add_trace(go.Scatter(
            x=years, y=margins, name="순이익률(%)",
...
def _chart_margin_trend(fin, chart_key="margin_trend"):
    if not HAS_PLOTLY:
        return
    try:
        ...
        if len(idx) < 2:
            st.caption("📈 마진 데이터가 부족합니다.")
            return

        years = []
        margins = []
        for i in idx:
            yr = i.strftime('%Y') if hasattr(i, 'strftime') else str(i)[:4]
            r_val = float(rs[i])
            n_val = float(ns[i])
            if r_val != 0:
                years.append(yr)
                margins.append(n_val / r_val * 100)
        ...
        fig.add_trace(go.Scatter(
            ...
            fill="tozeroy", fillcolor="rgba(130,170,255,0.1)",
        ...
        layout.update(
            ...
            yaxis=dict(title="순이익률(%)", gridcolor="#21262d",
                       zeroline=True, zerolinecolor="#FF174466", zerolinewidth=1),
            xaxis=dict(gridcolor="#21262d"),
        )
        fig.update_layout(**layout)
        st.plotly_chart(fig, use_container_width=True, key=chart_key)
...
def _chart_debt_donut(cash, curr_d, lt_d, eq, chart_key="debt_donut"):
    if not HAS_PLOTLY:
        return
    try:
        labels = ["보유 현금", "단기 부채", "장기 부채", "자본(Equity)"]
        values = [max(0, float(cash or 0)), max(0, float(curr_d or 0)),
                  max(0, float(lt_d or 0)), max(0, float(eq or 0))]
        colors = ["#00E676", "#FF9800", "#FF5722", "#2196F3"]
        if sum(values) == 0:
            st.caption("🍩 자산 구성 데이터가 없습니다.")
            return
...
def _chart_volume(vol, avg10, avg3m, chart_key="vol_chart"):
    if not HAS_PLOTLY:
        return
    try:
        labels = ["현재", "10일 평균", "3개월 평균"]
        values = [float(vol or 0), float(avg10 or 0), float(avg3m or 0)]
        colors = ["#82aaff", "#607D8B", "#444c56"]
        fig = go.Figure(data=[go.Bar(
            x=labels, y=values, marker_color=colors,
            text=[_fmt_num(v, False) for v in values],
            textposition="outside", textfont=dict(color="#c9d1d9")
        )])
        layout = _plotly_base_layout()
        layout.update(height=240,
...
def _chart_price_history(tkr, chart_key="price_hist"):
    if not HAS_PLOTLY:
        return
    try:
        ...
        layout = _plotly_base_layout()
        layout.update(height=220, showlegend=False,
...
def _growth_stage(info, fin, bs, cf):
    ...
    div_y = float(info.get('dividendYield', 0) or 0)
    ...
    stages = {
        1: ("🌱 1단계 : 스타트업 / 개발",
            "매출 미미·R&D 집중. 시장 검증 전, 높은 리스크·높은 잠재력."),
        2: ("🚀 2단계 : 초기 고성장",
            "매출 폭발적 증가, 아직 적자. 시장점유율 확대에 올인하는 단계."),
        3: ("📈 3단계 : 고성장 흑자전환",
            "높은 매출 성장 + 이익 창출 시작. 성장·수익 균형점 진입."),
        4: ("💪 4단계 : 성숙한 성장",
            "안정적 매출 성장 + 높은 수익성. 우량 성장주의 전형."),
        5: ("💰 5단계 : 캐시카우",
            "성장 둔화, 현금 창출 극대화. 배당·자사주매입 등 주주환원 활발."),
        6: ("⏸️ 6단계 : 정체기",
            "매출 성장 멈춤, 이익 유지. 새 성장 동력이 필요한 시점."),
        7: ("📉 7단계 : 쇠퇴기",
            "매출·이익 동반 하락. 구조적 변화 없이는 위험 증가."),
        8: ("🔧 8단계 : 구조조정 / 턴어라운드",
            "적극적 사업 재편·회생 시도. 성공 시 큰 반등 가능."),
    }
    colors = {1: "#9C27B0", 2: "#FF5722", 3: "#FF9800", 4: "#4CAF50",
              5: "#2196F3", 6: "#607D8B", 7: "#F44336", 8: "#795548"}

    if rev < 1e8 and margin < -0.20:
        s = 1
    elif rev_g > 0.30 and margin < 0:
        s = 2
    elif rev_g > 0.20 and margin > 0:
        s = 3
    elif rev_g > 0.05 and margin > 0.10:
        s = 4
    elif 0 <= rev_g <= 0.05 and margin > 0.10 and (div_y > 0.01 or (rev > 0 and op_cf > rev * 0.1)):
        s = 5
    elif -0.05 <= rev_g <= 0.02 and margin > 0:
        s = 6
    elif rev_declining and margin < 0:
        s = 7
    elif rev_g < -0.05 and margin <= 0:
        s = 8
    elif rev_g < 0 and margin <= 0:
        s = 8
    elif rev_g > 0.30:
        s = 2
    elif margin > 0.15 and rev_g > 0.05:
        s = 4
    elif margin > 0.15:
        s = 5
    elif margin > 0:
...
def _margin_trend(fin):
    try:
        ...
        for i in idx:
            r_val = float(rs[i])
            if r_val != 0:
                margins.append((i, float(ns[i]) / r_val))
        if len(margins) < 2:
            return "N/A", [], "gray"
        f_m = margins[0][1]
        l_m = margins[-1][1]
        if l_m > f_m + 0.02:
            return f"증가 📈 ({f_m * 100:.1f}%→{l_m * 100:.1f}%, 과거→현재)", margins, "green"
        if l_m < f_m - 0.02:
            return f"감소 📉 ({f_m * 100:.1f}%→{l_m * 100:.1f}%, 과거→현재)", margins, "red"
...
def _growth_accel(fin, rc):
    ...
    if len(g) < 2:
        return "N/A", "gray"
    if g[0] > g[1]:
        return f"가속화 🚀 (최근 {g[0] * 100:.1f}% vs 이전 {g[1] * 100:.1f}%)", "green"
    return f"감속 🐌 (최근 {g[0] * 100:.1f}% vs 이전 {g[1] * 100:.1f}%)", "yellow"


def _debt_trend(bs):
    for nc in [['Total Debt'], ['Long Term Debt'],
...
def _interest_burden(fin, info):
    ...
    if ebit is not None and interest is not None:
        try:
            ebit_f = float(ebit)
            int_f = float(interest)
            if abs(int_f) > 0:
                if ebit_f < 0:
                    return "위험 ❌ (영업적자)", "red"
                icr = abs(ebit_f / int_f)
                src = "SEC 실제"
    ...
    if ebitda is not None and debt is not None:
        try:
            ...
            if debt_f > 0:
                ...
                if ebitda_f > 0:
                    est = 0.05
                    cov = ebitda_f / (debt_f * est)
                    src = f"추정, 이자율 {est * 100:.0f}% 가정"
...
def _vol_trend(info):
    ...
    avg3m = float(info.get('averageVolume', 0) or 0)
    if avg3m == 0:
        return "데이터 부족", "gray"
    r = vol / avg3m
...
def _max_pain(tkr):
    try:
        ...
        c_oi = c['openInterest'].sum()
        p_oi = p['openInterest'].sum()
        if c_oi == 0 and p_oi == 0:
            wc = c['volume']
            wp = p['volume']
            if wc.sum() == 0 and wp.sum() == 0:
                return exp, None, [], []
        ...
        for s in strikes:
            pain[s] = float(
                np.sum(wc * np.maximum(0, s - c['strike']))
                + np.sum(wp * np.maximum(0, p['strike'] - s))
            )
        mp = None
        if pain and max(pain.values()) > 0:
...
SECTOR_PE_FB = {"Technology": 30, "Communication Services": 22, "Consumer Cyclical": 25,
...
def _sector_pe_live(sector):
    ...
    return (float(fb), "추정치") if fb else (None, "")
...
def render_company_details(ticker_str: str):
    ...
    with st.spinner(f"📡  {ticker_str}  분석 중 …"):
        ...
        try:
            ...
            price = 0.0
        prev_c = float(info.get('previousClose', 0) or 0) or price or 1.0
    ...
    st.markdown(f"""<div class="header-wrap"><div>
    ...
    <span style="font-size:.85rem;color:#8b949e">{_esc(sector)} · {_esc(industry)}</span></div>
    <div style="text-align:right">
    <span style="font-size:1.7rem;font-weight:800;color:{chg_c}">${price:,.2f}</span><br>
    <span style="font-size:.9rem;color:{chg_c}">{chg_s}{day_chg:.2f}% 오늘</span></div></div>""",
    ...
    v1_c = ("green" if sn in [3, 4] else
            ("blue" if sn == 5 else
             ("yellow" if sn in [1, 2, 6] else
              ("red" if sn in [7, 8] else "gray"))))
    ...
    <div style="text-align:center;margin:16px 0"><span style="display:inline-block;padding:10px 28px;border-radius:24px;font-weight:700;background:{scolor};color:#fff;font-size:1.1rem;box-shadow:0 4px 16px {scolor}44">{sname}</span>
    <div style="font-size:.88rem;color:#8b949e;margin-top:12px;line-height:1.7">{sdesc}</div></div>
    <div class="divider"></div>
    <div class="two-col"><div>{_metric_row("분기 매출 성장률 (YoY)", _fmt_pct(info.get('revenueGrowth')))}{_metric_row("순이익률", _fmt_pct(info.get('profitMargins')))}</div>
    <div>{_metric_row("배당수익률", _fmt_pct(info.get('dividendYield')))}{_metric_row("ROE", _fmt_pct(info.get('returnOnEquity')))}</div></div>
    <div class="note-box">💡 성장률은 Yahoo Finance 기준 <b>분기 YoY</b> 수치입니다.</div>
    ...
    ps = sum(1 for x in [gross_m and gross_m > 0.4, net_m and net_m > 0.1,
                          cagr_rev and cagr_rev > 0.05, cagr_ni and cagr_ni > 0.05,
                          op_m and op_m > 0.15] if x)
    ...
    gm_pct = float(gross_m or 0) * 100
    nm_pct = float(net_m or 0) * 100
    ...
    <div class="two-col"><div>{_metric_row("시가총액", _fmt_num(mcap), "m-value m-big")}{_metric_row("TTM EPS", f"${_safe(ttm_eps)}")}{_metric_row("최근 12개월 매출", _fmt_num(ttm_rev))}{_metric_row("최근 12개월 순이익", _fmt_num(ttm_ni), ni_cls)}</div>
    <div><div style="margin-bottom:12px"><div style="display:flex;justify-content:space-between;font-size:.88rem"><span style="color:#8b949e">총이익률</span><span style="color:#c9d1d9;font-weight:700">{_fmt_pct(gross_m)}</span></div>{_gauge_bar(max(0, gm_pct), "#4CAF50", 10)}</div>
    <div style="margin-bottom:12px"><div style="display:flex;justify-content:space-between;font-size:.88rem"><span style="color:#8b949e">순이익률</span><span style="color:#c9d1d9;font-weight:700">{_fmt_pct(net_m)}</span></div>{_gauge_bar(max(0, nm_pct), "#2196F3", 10)}</div>
    {_metric_row("영업이익률", _fmt_pct(op_m))}</div></div>
    <div class="divider"></div>
    ...
    with ch1:
        ...
        _chart_revenue_profit(fin, "chart_rev_profit")
    with ch2:
        st.caption("📈 순이익률 추이")
        _chart_margin_trend(fin, "chart_margin")
    ...
    if roe_val is not None:
        try:
            rv_f = float(roe_val)
            if rv_f > 0.20:   roe_lbl, roe_c = f"매우 높음 ✅ ({_fmt_pct(roe_val)})", "green"
            elif rv_f > 0.10: roe_lbl, roe_c = f"양호 ✅ ({_fmt_pct(roe_val)})", "green"
            elif rv_f > 0:    roe_lbl, roe_c = f"보통 ⚠️ ({_fmt_pct(roe_val)})", "yellow"
    ...
    if s3 >= 3:   v3_c, v3_t = "green", "✅ 우수한 트랙 레코드"
    elif s3 >= 2: v3_c, v3_t = "yellow", "⚠️ 보통 — 일부 양호, 개선 여지"
    else:         v3_c, v3_t = "red", "❌ 주의 — 수익 불안정 또는 하락 추세"
    all_verdicts.append(("과거성적", v3_c))

    st.markdown(f"""<div class="s-card"><div class="s-title"><span class="s-num">03</span> 지금까지 성적 <span style="font-size:.75rem;color:#6e7681">SEC</span></div>
    <div class="two-col"><div>{_metric_row("수익 안정성", f'{rev_stab} <span style="font-size:.75rem;color:#555">{rev_cv}</span>')}{_metric_row("이익 마진 추이", mtrend)}{_metric_row("성장 가속화", accel)}{_metric_row("ROE 수준", roe_lbl)}</div>
    <div><table class="m-table"><tr><th>연도</th><th>매출</th><th>순이익</th><th>순이익률</th></tr>{tbl}</table></div></div>
    {_verdict_badge(v3_c, "📌", v3_t)}</div>""", unsafe_allow_html=True)

    # ═══════════════════════════ 4️⃣ 성장 가능성 ════════════════
    payout = float(info.get('payoutRatio', 0) or 0)
    ...
    if gs >= 4:   v4_c, v4_t = "green", "🚀 높은 성장 잠재력"
    elif gs >= 2: v4_c, v4_t = "yellow", "⚠️ 보통 — 모니터링 필요"
    else:         v4_c, v4_t = "red", "❌ 성장 둔화"
    ...
    <div class="two-col"><div>{_metric_row("분기 EPS 성장률 (YoY)", _fmt_pct(eg), eg_cls)}{_metric_row("분기 매출 성장률 (YoY)", _fmt_pct(rg), rg_cls)}{_metric_row("Forward EPS", f"${_safe(fwd_eps)}")}{_metric_row("PEG 비율", _safe(peg), "m-green" if peg and 0 < peg < 1.5 else "m-value")}</div>
    <div>{_metric_row("ROE", _fmt_pct(roe_v))}{_metric_row("수익 유보율 (1-배당성향)", _fmt_pct(retention))}{_metric_row("지속가능 성장률 (g=ROE×유보율)", _fmt_pct(sust_g), "m-green" if sust_g and sust_g > 0.1 else "m-value")}
    <div class="note-box">💡 <b>지속가능 성장률(g)</b>: 외부 자금 없이 내부 유보만으로 달성 가능한 이론적 최대 성장률.</div></div></div>
    {_verdict_badge(v4_c, "📌", v4_t)}</div>""", unsafe_allow_html=True)

    # ═══════════════════════════ 5️⃣ 재무 건전성 ════════════════
    try:
        curr_d = float(_get_row(bs, ['Current Debt', 'Current Portion Of Long Term Debt']) or 0)
        lt_d = float(_get_row(bs, ['Long Term Debt', 'Long Term Debt And Capital Lease Obligation']) or 0)
        ta = float(_get_row(bs, ['Total Assets']) or info.get('totalAssets', 0) or 0)
        tl = float(_get_row(bs, ['Total Liabilities Net Minority Interest', 'Total Liab']) or 0)
        eq = float(_get_row(bs, ['Stockholders Equity', 'Total Stockholder Equity']) or 0)
        na = ta - tl
    except Exception:
        curr_d = lt_d = ta = tl = eq = na = 0.0
    ...
    if hs >= 3:   v5_c, v5_t = "green", "💪 재무 건전"
    elif hs >= 2: v5_c, v5_t = "yellow", "⚠️ 보통 — 모니터링 필요"
    else:         v5_c, v5_t = "red", "❌ 주의 — 부채 높거나 현금 부족"
    all_verdicts.append(("재무건전", v5_c))

    eq_pct = max(0, eq / ta * 100) if ta > 0 else 0
    li_pct = max(0, tl / ta * 100) if ta > 0 else 0
    ...
    <div class="two-col"><div>{_metric_row("💵 보유 현금", _fmt_num(cash), "m-value m-green m-big")}{_metric_row("단기 부채", _fmt_num(curr_d))}{_metric_row("장기 부채", _fmt_num(lt_d))}
    <div class="divider"></div>{_metric_row("부채 수준 (D/E)", dl)}{_metric_row("부채 추세", dt_trend)}{_metric_row("이자 부담 (ICR)", ib_txt)}</div>
    <div>{_metric_row("총 자산", _fmt_num(ta))}{_metric_row("총 부채", _fmt_num(tl))}{_metric_row("자본 (주주 지분)", _fmt_num(eq))}{_metric_row("순 자산", _fmt_num(na), "m-green" if na > 0 else "m-red")}
    <div class="divider"></div><div style="font-size:.82rem;color:#6e7681;margin-bottom:6px;font-weight:600">자산 구조</div>
    ...
    <div class="note-box">※ D/E 판정은 섹터({_esc(sector)}) 특성을 반영합니다.</div></div></div>
    {_verdict_badge(v5_c, "📌", v5_t)}</div>""", unsafe_allow_html=True)

    # 📊 도넛 차트
    st.caption("🍩 자산 구성")
    _chart_debt_donut(cash, curr_d, lt_d, eq, "chart_donut")

    # ═══════════════════════════ 6️⃣ 거래량 ═════════════════════
    vol = float(info.get('volume', 0) or 0)
    avg10 = float(info.get('averageVolume10days', 0) or 0)
    avg3m = float(info.get('averageVolume', 0) or 0)
    vt_txt, vt_c = _vol_trend(info)
    v6_t = {"green": "✅ 거래량 정상", "yellow": "⚠️ 거래량 변동", "red": "🔥 거래량 이상"}.get(vt_c, "데이터 부족")
    all_verdicts.append(("거래량", vt_c))

    st.markdown(f"""<div class="s-card"><div class="s-title"><span class="s-num">06</span> 사람들이 많이 사고 있나요? <span style="font-size:.75rem;color:#6e7681">Yahoo</span></div>
    <div class="two-col"><div>{_metric_row("현재 거래량", f"{_fmt_num(vol, False)} 주")}{_metric_row("10일 평균", f"{_fmt_num(avg10, False)} 주")}{_metric_row("3개월 평균", f"{_fmt_num(avg3m, False)} 주")}
    <div class="divider"></div>{_metric_row("거래량 변동 추이", vt_txt)}</div><div></div></div>
    {_verdict_badge(vt_c, "📌", v6_t)}</div>""", unsafe_allow_html=True)

    st.caption("📊 거래량 비교")
    _chart_volume(vol, avg10, avg3m, "chart_vol")
    ...
    if isinstance(beta, (int, float)):
        try:
            beta_f = float(beta)
            if beta_f < 0.8:   bl, bc = "저변동 — 시장보다 안정", "green"
            elif beta_f < 1.2: bl, bc = "시장과 유사", "yellow"
            elif beta_f < 1.5: bl, bc = "높은 변동성 ⚠️", "red"
            else:              bl, bc = "매우 높은 변동성 🔥", "red"
        except (ValueError, TypeError):
            bl, bc = "N/A", "gray"
    else:
        bl, bc, beta_f = "N/A", "gray", None
    ...
    <div class="two-col"><div>{_metric_row("베타 (β)", f"{beta_f:.2f} ({bl})" if beta_f is not None else "N/A")}{_metric_row("52주 가격 변화율", _fmt_pct(w52c), "m-green" if w52c and w52c > 0 else "m-red")}{_metric_row("52주 최고가", f"${float(w52h):,.2f}" if w52h else "N/A")}{_metric_row("52주 최저가", f"${float(w52l):,.2f}" if w52l else "N/A")}{_metric_row("현재가 위치 (52주 내)", pos_str)}</div>
    <div><div style="font-size:.82rem;color:#6e7681;margin-bottom:4px;font-weight:600">📍 52주 범위 내 위치</div>{pos_bar}
    <div class="note-box">💡 베타 &lt; 1 = 시장(S&amp;P500) 대비 안정적 / 베타 &gt; 1 = 시장보다 변동성 큼</div></div></div>
    {_verdict_badge(bc, "📌", v7_t)}</div>""", unsafe_allow_html=True)

    st.caption("📈 최근 1년 주가 추이")
    _chart_price_history(tkr, "chart_price")
    ...
    if t_pe and s_pe:
        try:
            tpf = float(t_pe)
            if tpf < s_pe * 0.8:   v8_c, v8_t = "green", "💎 저평가 가능성"
            elif tpf < s_pe * 1.3: v8_c, v8_t = "yellow", "⚖️ 적정 수준"
    ...
    <div class="two-col"><div>{_metric_row("Trailing P/E", f"{float(t_pe):.2f}" if t_pe else "N/A")}{_metric_row("Forward P/E", f"{float(f_pe):.2f}" if f_pe else "N/A")}{_metric_row("P/S (TTM)", f"{float(p_s):.2f}" if p_s else "N/A")}{_metric_row("P/B", f"{float(p_b):.2f}" if p_b else "N/A")}
    <div class="divider"></div>{_metric_row(f"섹터 ({_esc(sector)})", pe_lbl)}{_metric_row("섹터 대비", pe_comp if pe_comp else "N/A")}</div>
    <div>{pe_visual}<div class="note-box">※ 섹터 P/E는 {pe_source} 기준.</div></div></div>
    ...
    if isinstance(rm, (int, float)):
        try:
            rmf = float(rm)
            if rmf <= 1.5:   con, cc = "🟢 강력 매수", "green"
            elif rmf <= 2.0: con, cc = "🟢 매수", "green"
            elif rmf <= 2.5: con, cc = "🟡 매수 우위", "yellow"
            elif rmf <= 3.0: con, cc = "🟡 보유", "yellow"
            elif rmf <= 3.5: con, cc = "🟠 매도 우위", "red"
            elif rmf <= 4.0: con, cc = "🔴 매도", "red"
    ...
    try:
        if t_median and price and price > 0:
            up_pct = ((float(t_median) - price) / price) * 100
    ...
    <div class="two-col"><div>{_metric_row("컨센서스 등급", rk, "m-value m-blue m-big")}{_metric_row("평균 의견 (1매수~5매도)", f"{float(rm):.2f}" if rm else "N/A")}{_metric_row("종합 의견", con)}{_metric_row("참여 애널리스트", f"{n_ana}명")}
    <div class="divider"></div>{_metric_row("평균 목표가", f"${_safe(t_mean)}")}{_metric_row("중앙값 목표가", f"${_safe(t_median)}")}{_metric_row("최고 목표가", f"${_safe(t_high)}")}{_metric_row("최저 목표가", f"${_safe(t_low)}")}</div>
    <div>{target_bar}<div style="text-align:center;margin-top:16px;padding:14px;background:rgba(0,0,0,.2);border-radius:12px;border:1px solid #2D333B"><div style="font-size:.82rem;color:#8b949e">목표가 대비 여력</div><div style="font-size:1.5rem;font-weight:800;color:{'#00E676' if up_pct > 0 else '#FF1744'};margin-top:4px">{up_str}</div></div></div></div>
    {_verdict_badge(cc, "📌", f"애널리스트 {n_ana}명 {rk} ({up_str})")}</div>""", unsafe_allow_html=True)

    # ═══════════════════════════ 🔟 지분 구조 ═══════════════════
    inst = float(info.get('heldPercentInstitutions', 0) or 0)
    ins = float(info.get('heldPercentInsiders', 0) or 0)
    pub = max(0.0, 1.0 - inst - ins)
    s_out = float(info.get('sharesOutstanding', 0) or 0)
    s_flt = float(info.get('floatShares', 0) or 0)
    ...
    <div class="two-col"><div>{_metric_row("총 발행 주식수", f"{_fmt_num(s_out, False)} 주")}{_metric_row("유통 주식수 (Float)", f"{_fmt_num(s_flt, False)} 주")}
    <div class="divider"></div>{_metric_row("🏛️ 기관 (연기금·펀드·정부기관 포함)", _fmt_pct(inst))}{_metric_row("👔 내부자 (임원·이사회)", _fmt_pct(ins))}{_metric_row("👤 개인/기타 (추정)", _fmt_pct(pub))}</div>
    <div><div style="font-size:.82rem;color:#6e7681;margin-bottom:8px;font-weight:600">📊 지분 구성</div>
    <div class="p-container" style="height:36px;border-radius:12px"><div class="p-bar" style="width:{inst*100:.1f}%;background:linear-gradient(135deg,#1565C0,#42A5F5)">{'🏛️ '+str(round(inst*100))+'%' if inst>0.08 else ''}</div><div class="p-bar" style="width:{ins*100:.1f}%;background:linear-gradient(135deg,#E65100,#FF9800)">{'👔 '+str(round(ins*100))+'%' if ins>0.04 else ''}</div><div class="p-bar" style="width:{pub*100:.1f}%;background:linear-gradient(135deg,#2E7D32,#66BB6A)">{'👤 '+str(round(pub*100))+'%' if pub>0.08 else ''}</div></div>
    ...
    <div class="note-box">※ '기관'에는 정부 연기금, 뮤추얼펀드, ETF, 헤지펀드 포함.<br>※ '개인/기타'는 (100%-기관-내부자) 추정값.</div></div></div>
    ...
    if mp and price and price > 0:
        ...
        if d > 2:
            mp_note = f"Max Pain이 현재가보다 {d:+.1f}% 위 → 만기 시 상승 수렴 가능성"
            mp_c = "green"
        elif d < -2:
            mp_note = f"Max Pain이 현재가보다 {d:+.1f}% 아래 → 만기 시 하락 수렴 가능성"
            mp_c = "red"
        else:
            mp_note = f"현재가와 Max Pain 유사 ({d:+.1f}%) → 현재 가격대 유지"
            mp_c = "yellow"
    ...
    for ci in (tc or []):
        try:
            ch_html += f"<li>${float(ci['strike']):.1f} <span style='color:#6e7681'>(Vol {int(ci['volume']):,})</span></li>"
        except Exception:
            pass
    if not ch_html:
        ch_html = "<li>N/A</li>"
    ...
    for pi in (tp or []):
        try:
            ph_html += f"<li>${float(pi['strike']):.1f} <span style='color:#6e7681'>(Vol {int(pi['volume']):,})</span></li>"
        except Exception:
            pass
    if not ph_html:
        ph_html = "<li>N/A</li>"
    ...
    <div class="divider"></div>
    <div style="display:flex;gap:16px;flex-wrap:wrap"><div class="opt-box" style="flex:1;min-width:220px"><b style="color:#00E676;font-size:.9rem">🟢 Call Top 3</b><ul class="opt-list">{ch_html}</ul></div><div class="opt-box" style="flex:1;min-width:220px"><b style="color:#FF1744;font-size:.9rem">🔴 Put Top 3</b><ul class="opt-list">{ph_html}</ul></div></div>
    <div class="note-box" style="margin-top:14px">💡 <b>Max Pain</b>: 옵션 만기 시 가장 많은 옵션 매수자가 손실을 보는 가격. 주가가 만기일에 이 가격으로 수렴하는 경향이 있다는 이론입니다.<br>※ 참고 지표이며 항상 수렴하지는 않습니다.</div>
    {_verdict_badge(mp_c, "📌", mp_badge)}</div>""", unsafe_allow_html=True)

    # ═══════════════════════════ 1️⃣2️⃣ 공매도 ═══════════════════
    sp = info.get('shortPercentOfFloat')
    ss = float(info.get('sharesShort', 0) or 0)
    sr = info.get('shortRatio')
    pss = float(info.get('sharesShortPriorMonth', 0) or 0)

    if isinstance(sp, (int, float)):
        spf = float(sp)
        if spf > 0.20:   sl, sc = "매우 높음 🔴 (숏스퀴즈 가능)", "red"
        elif spf > 0.10: sl, sc = "높음 🟠", "red"
        elif spf > 0.05: sl, sc = "보통 🟡", "yellow"
        else:            sl, sc = "낮음 🟢", "green"
    else:
        sl, sc = "N/A", "gray"
    ...
    sp_pct = float(sp or 0) * 100
    sp_color = "#FF1744" if sp_pct > 10 else ("#FFC107" if sp_pct > 5 else "#00E676")
    ...
    <div class="two-col"><div>{_metric_row("유통주식 대비 공매도", _fmt_pct(sp), "m-red" if sc == "red" else "m-value")}
    <div style="font-size:.78rem;color:#6e7681;margin-bottom:4px">공매도 게이지 (20%=가득 참)</div>{_gauge_bar(min(sp_pct * 5, 100), sp_color, 12)}
    {_metric_row("공매도 수준", sl)}
    {('<div class="m-row"><span class="m-label">전월 대비</span><span class="m-value">' + short_trend + '</span></div>') if short_trend else ''}</div>
    <div>{_metric_row("공매도 주식수", f"{_fmt_num(ss, False)} 주")}{_metric_row("발행 주식수", f"{_fmt_num(s_out, False)} 주")}{_metric_row("숏커버 일수 (DTC)", f"{float(sr):.2f} 일" if sr else "N/A")}
    <div class="note-box">💡 공매도 &gt; 10% = 높은 숏 관심 / DTC 높으면 숏스퀴즈 가능성 ↑<br>※ 공매도 데이터는 약 2주 지연.</div></div></div>
    ...
    try:
        ...
        if news_list and len(news_list) > 0:
            items = ""
            for n_item in news_list[:8]:
                title = n_item.get('title') or n_item.get('content', {}).get('title', '제목 없음')
                link = n_item.get('link') or n_item.get('content', {}).get('canonicalUrl', {}).get('url', '#')
                pub_name = n_item.get('publisher') or n_item.get('content', {}).get('provider', {}).get('displayName', '')
                ts = n_item.get('providerPublishTime', 0)
                if not ts:
                    pd_s = n_item.get('content', {}).get('pubDate', '')
                    dt_s = pd_s[:16] if pd_s else ""
                else:
                    dt_s = datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M') if ts else ""
            ...
            st.markdown(f"""<div class="s-card"><div class="s-title"><span class="s-num">13</span> 최신 뉴스 <span style="font-size:.75rem;color:#6e7681">Yahoo</span></div>{items}</div>""", unsafe_allow_html=True)
        else:
            raise ValueError
    except Exception:
    ...
    <div class="note-box" style="margin-top:12px;text-align:center">핵심 지표(수익성·성장성·재무건전)에 높은 가중치를 부여한 종합 점수. 데이터 누락 항목은 제외.</div></div>""", unsafe_allow_html=True)

    st.caption("⚠️ 본 분석은 참고용이며 투자 조언이 아닙니다.")
...
if __name__ == "__main__":
    ...
    if not HAS_PLOTLY:
        st.warning("⚠️ plotly가 설치되지 않았습니다. `pip install plotly` 실행 후 차트가 표시됩니다.")
