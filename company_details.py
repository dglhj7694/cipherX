...
def _get_growth_stage(info, cagr_rev):
    ...
    if rev_growth > 0.4 and net_margin < 0: return "초기 고성장 (매출 폭발, 적자 지속)"
    elif rev_growth > 0.2 and net_margin > 0: return "고성장 흑자 (매출 고성장, 이익 창출)"
    elif rev_growth > 0.1 and net_margin > 0.1: return "성숙한 성장 (안정적 매출, 고수익성)"
    elif rev_growth > 0 and net_margin > 0.15: return "캐시카우 (성장 둔화, 막대한 현금흐름)"
    elif rev_growth > 0 and net_margin < 0: return "스타트업/개발 (미미한 매출, 투자 단계)"
    elif rev_growth <= 0 and net_margin > 0: return "정체기 (매출 감소, 이익 유지)"
    elif rev_growth <= -0.1 and net_margin < 0: return "쇠퇴기 (역성장 및 적자 누적)"
    elif rev_growth <= 0 and net_margin <= 0: return "구조조정 (매출/이익 동반 하락)"
    return "평가 불가 (데이터 부족)"
...
def render_company_details(ticker_str):
    st.markdown("""
    <style>
    .info-card { background-color: #161A22; border: 1px solid #2D333B; border-radius: 12px; padding: 22px; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); transition: transform 0.2s ease; }
    ...
    .opt-box { background: rgba(0,0,0,0.2); padding: 10px; border-radius: 8px; border: 1px solid #2D333B; }
    .opt-list { margin: 5px 0 0 0; padding-left: 20px; font-size: 0.85rem; color: #c9d1d9; }
    </style>
    """, unsafe_allow_html=True)
    
    with st.spinner(f"📡 {ticker_str}의 SEC 공시 및 파생 데이터를 딥 다이빙 중입니다..."):
    ...
    st.markdown(f"### 🏢 {info.get('shortName', ticker_str)} <span style='font-size:1.1rem; color:#888;'>| {sector} · {industry}</span>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown(f"""
        <div class="info-card">
            <div class="info-title">🌱 기업 성장 주기 및 수익성 (SEC)</div>
            <div class="metric-row"><span class="metric-label">성장 사이클:</span> <span class="metric-value metric-highlight">{_get_growth_stage(info, cagr_rev)}</span></div>
            <div class="metric-row"><span class="metric-label">시가총액:</span> <span class="metric-value">{_fmt_num(info.get('marketCap'))}</span></div>
            <div class="metric-row"><span class="metric-label">주당 순이익(TTM EPS):</span> <span class="metric-value">${info.get('trailingEps', 'N/A')}</span></div>
            ...
            <div class="metric-row"><span class="metric-label">5년 매출 연평균성장률:</span> <span class="metric-value">{_fmt_pct(cagr_rev)}</span></div>
            <div class="metric-row"><span class="metric-label">5년 순이익 연평균성장률:</span> <span class="metric-value">{_fmt_pct(cagr_ni)}</span></div>
            <div class="metric-row"><span class="metric-label">5년 EPS 연평균성장률:</span> <span class="metric-value">{_fmt_pct(cagr_eps)}</span></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div class="info-card">
            <div class="info-title">🏦 대차대조표 및 재무 건전성</div>
        ...
        st.markdown(f"""
        <div class="info-card">
            <div class="info-title">📊 수급, 변동성 및 지분 구조</div>
            ...
            <div class="metric-row"><span class="metric-label">시장 유통 주식수(Float):</span> <span class="metric-value">{_fmt_num(info.get('floatShares'), False)} 주</span></div>
            ...
            <div class="metric-row"><span class="metric-label">내부자(임원) 비중:</span> <span class="metric-value">{_fmt_pct(info.get('heldPercentInsiders'))}</span></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown(f"""
        <div class="info-card">
            <div class="info-title">🚀 최근 실적 및 미래 성장 동력</div>
            <div class="metric-row"><span class="metric-label">전년 대비 이익 성장(YOY):</span> <span class="metric-value">{_fmt_pct(info.get('earningsGrowth'))}</span></div>
            <div class="metric-row"><span class="metric-label">전년 대비 매출 성장(YOY):</span> <span class="metric-value">{_fmt_pct(info.get('revenueGrowth'))}</span></div>
            <div class="metric-row"><span class="metric-label">자기자본이익률(ROE):</span> <span class="metric-value metric-highlight">{_fmt_pct(roe)}</span></div>
            <div class="divider"></div>
            <div class="metric-row"><span class="metric-label">수익 대비 유보율(저축률):</span> <span class="metric-value">{_fmt_pct(retention_ratio)}</span></div>
            <div class="metric-row"><span class="metric-label">미래 예상 ROE:</span> <span class="metric-value">{_fmt_pct(future_roe_est)}</span></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div class="info-card">
            <div class="info-title">⚖️ 밸류에이션 및 월가 컨센서스</div>
            ...
            <div class="divider"></div>
            <div class="metric-row"><span class="metric-label">월가 종합 의견:</span> <span class="metric-value metric-highlight">{str(info.get('recommendationKey', 'N/A')).upper()}</span></div>
            <div class="metric-row"><span class="metric-label">평견 의견 점수 (1=매수~5=매도):</span> <span class="metric-value">{info.get('recommendationMean', 'N/A')}</span></div>
        ...
        pain_html = f"<span class='metric-value metric-warn' style='font-size:1.1rem;'>${pain}</span>" if pain else "<span class='metric-value'>데이터 없음</span>"
        date_html = f"({date})" if date else ""
        
        c_html = "".join([f"<li>${c['strike']} (Vol: {c['volume']})</li>" for c in top_c]) if top_c else "<li>데이터 없음</li>"
        p_html = "".join([f"<li>${p['strike']} (Vol: {p['volume']})</li>" for p in top_p]) if top_p else "<li>데이터 없음</li>"

        st.markdown(f"""
        <div class="info-card">
            <div class="info-title">⚠️ 숏(공매도) 및 옵션 파생 심리</div>
            ...
            <div class="metric-row"><span class="metric-label">현재 공매도 잔고 (Shares):</span> <span class="metric-value">{_fmt_num(info.get('sharesShort'), False)} 주</span></div>
            <div class="metric-row"><span class="metric-label">숏 커버링 필요 일수 (DTC):</span> <span class="metric-value">{info.get('shortRatio', 'N/A')} 일</span></div>
            <div class="divider"></div>
            <div class="metric-row"><span class="metric-label">옵션 Max Pain 가격 {date_html}:</span> {pain_html}</div>
            
            <div style="display:flex; justify-content: space-between; margin-top: 15px;">
                <div class="opt-box" style="width:48%;">
                    <b style="color:#00E676; font-size:0.9rem;">🟢 Call 거래량 Top 3</b>
                    <ul class="opt-list">{c_html}</ul>
                </div>
                <div class="opt-box" style="width:48%;">
                    <b style="color:#FF1744; font-size:0.9rem;">🔴 Put 거래량 Top 3</b>
                    <ul class="opt-list">{p_html}</ul>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)