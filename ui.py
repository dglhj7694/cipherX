...
def render_judgment_card(m):
    ...
    veto_html=f"<div style='margin-top:6px'><span style='background:rgba(239,68,68,.15);color:#FCA5A5;padding:3px 8px;border-radius:6px;font-size:.7rem;font-weight:700'>[VETO] {veto}</span></div>" if veto else ""
    extra=""
    ...
    st.markdown(f"""<div class="score-card {cc} fade-up"><p style="font-size:2.2rem;font-weight:800;color:{jc};margin:0;letter-spacing:-1px;">{labels.get(jg,jg)}</p>
        <div style="display:flex;align-items:center;gap:10px;justify-content:center;margin-top:12px"><div style="flex:0 0 200px;height:10px;background:rgba(255,255,255,0.1);border-radius:5px;overflow:hidden;box-shadow:inset 0 1px 3px rgba(0,0,0,0.5)"><div style="width:{min(cf,100)}%;height:10px;background:{jc};border-radius:5px;box-shadow:0 0 10px {jc}80"></div></div><span style="color:{jc};font-weight:800;font-size:1.1rem">{cf:.0f}%</span></div>
        {reason_html}
        <div style="display:flex;justify-content:center;gap:32px;margin-top:18px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.05)"><div><p style="color:#64748B;font-size:.75rem;font-weight:700;margin:0">Ensemble</p><p style="color:{'#34D399' if es>0 else '#F87171' if es<0 else '#FCD34D'};font-size:1.5rem;font-weight:800;margin:2px 0">{es:+.1f}</p></div>
        <div style="border-left:1px solid rgba(255,255,255,.08);padding-left:32px"><p style="color:#64748B;font-size:.75rem;font-weight:700;margin:0">Agree</p><p style="color:#F8FAFC;font-size:1.5rem;font-weight:800;margin:2px 0">B{ba}:S{sa}</p></div>
        <div style="border-left:1px solid rgba(255,255,255,.08);padding-left:32px"><p style="color:#64748B;font-size:.75rem;font-weight:700;margin:0">Context</p><p style="color:#A5B4FC;font-size:1.2rem;font-weight:800;margin:2px 0">{m.get('context_label','기본')}</p></div></div>
        <div style='margin-top:12px'>{extra}</div>{veto_html}</div>""",unsafe_allow_html=True)
...
def render_committee_panel(m):
    ...
    st.markdown(f"#### Context: {m.get('context_label','기본')}")
    cols=st.columns(5)
    for ci,cm in enumerate(COMMITTEE_NAMES):
        ...
        with cols[ci]:st.metric(label=f"■ {cm} ×{weight:.0%}",value=f"{score:+.0f}",delta=f"{vote} ({conv:.0f}%)",delta_color="normal" if score>0 else "inverse" if score<0 else "off")
...
def render_leading_lagging(m):
    ...
    st.markdown(f"""<div style="background:rgba(255,255,255,.03);border:1px solid #1E293B;border-radius:12px;padding:14px;margin-bottom:10px"><p style="font-weight:700;color:#A5B4FC;margin:0 0 8px">Leading Indicator</p><p style="color:{lc};font-weight:800;font-size:1.1rem;margin:0">{lv}</p>
        <div style="display:flex;gap:10px;margin-top:8px;flex-wrap:wrap"><span style="color:#94A3B8;font-size:.8rem">Accel:<b style="color:{'#34D399' if ac>0 else '#F87171'}">{ac:+.2f}</b></span><span style="color:#94A3B8;font-size:.8rem">UT:{'Buy' if m.get('utbot_dir',0)==1 else('Sell' if m.get('utbot_dir',0)==-1 else 'Neutral')}</span><span style="color:#94A3B8;font-size:.8rem">Hull:{'Up' if m.get('hma_rising') else 'Down'}</span></div></div>""",unsafe_allow_html=True)
    st.markdown(f"""<div style="background:rgba(255,255,255,.03);border:1px solid #1E293B;border-radius:12px;padding:14px"><p style="font-weight:700;color:#A5B4FC;margin:0 0 8px">Lagging Indicator</p><p style="color:{lgc};font-weight:800;font-size:1.1rem;margin:0">{lgv}</p>
        <div style="display:flex;gap:16px;margin-top:8px"><span style="color:#94A3B8;font-size:.8rem">Context:<b>{m['regime_label']}</b></span><span style="color:#94A3B8;font-size:.8rem">RS:<b style="color:{'#34D399' if m['rs_ratio']>1.03 else('#F87171' if m['rs_ratio']<.97 else '#FCD34D')}">{m['rs_ratio']:.3f}</b></span></div></div>""",unsafe_allow_html=True)
...
def render_analysis(msg):
    ...
    if m or fj:
        ...
        with t2:
            if m:st.markdown("#### 10-Layer Score");render_10layer_bars(m)
        with t3:
